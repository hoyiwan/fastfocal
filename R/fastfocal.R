...
#' @param fun character. "mean","sum","min","max","sd","median".
...
#'
#' @return [terra::SpatRaster] with same geometry as `x`.
#' @export
...
fastfocal <- function(
  ...
  if (terra::nlyr(x) > 1) {
    out_list <- lapply(seq_len(terra::nlyr(x)), function(i) {
      xi <- x[[i]]
      out_i <- fastfocal(xi, d = d, w = w, fun = fun,
  ...
  if (identical(engine, "fft")) {
    ...
    fft_mat <- fft_convolve_masked(mat, kernel, fun = fun, na.rm = na.rm, na.policy = na.policy, pad = pad)
    out <- terra::rast(x); terra::values(out) <- as.vector(fft_mat)
